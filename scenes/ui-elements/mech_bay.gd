...
func _on_pw_menu_item_selected(index: int) -> void:
	...
	PlayerConfig.primary_weapon = %PWMenu.get_item_text(select).to_snake_case()


func _on_sw_menu_item_selected(index: int) -> void:
	var select = %SWMenu.selected
	PlayerConfig.secondary_weapon = %SWMenu.get_item_text(select).to_snake_case()
	print(PlayerConfig.secondary_weapon)


func _on_body_menu_item_selected(index: int) -> void:
	var select = %BodyMenu.selected
	PlayerConfig.mech_body = %BodyMenu.get_item_text(select).to_snake_case()
